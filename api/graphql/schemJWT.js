...
const database = new sqlite3.Database("./db/project-management.db");

//Secretkey
const secretKey = "a6qgWWdfuzMhrBPQ7Afxj2PxDGrQ"
    ...
    fields: {
            ...
            resolve:  (root, {email,password}, context, info) => {
                return new Promise((resolve, reject) =>{
                    database.all("SELECT * FROM usuario WHERE email = (?);", [email],  async (err,row) => {
                       ...
                       let valid = await comparePassword(password, row[0]["password"])
                       if (!valid) {
                        reject({auth:false});
                      }
                       const token = jwt.sign({id: row[0]["id"], email: row[0]["email"]}, secretKey, {expiresIn: 60*60});
        ...
        Usuario: {
            ...
            resolve: (root, {email}, context, info) => {
                return new Promise((resolve, reject) =>{
                    database.all("SELECT * FROM usuario WHERE email = (?);", [email], function(err,rows) {
                       if(err){
                           reject(null);
                       } 
                       resolve(rows[0]);
                ...
                return new Promise((resolve, reject) =>{
                    database.run('INSERT INTO usuario (nombre, email, password, tipo) VALUES (?,?,?,?);', [nombre, email, pass, tipo], (err) => {
                        ...
                        database.get('SELECT last_insert_rowid() as id', (err, row) => {
                            const token = jwt.sign({id: row["id"]}, secretKey, {expiresIn: 60*60})
                            resolve({
                                auth: true,
                                id: row["id"],
                                token: token,
